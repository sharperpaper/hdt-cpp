 ...
 *
 *
 * Edited by Miel Vander Sande, 2016
 */
...
bool DictionaryEntry::cmpLexicographic(DictionaryEntry *c1,
                                       DictionaryEntry *c2) {
...
PlainDictionary::PlainDictionary() { this->mapping = MAPPING2; }

PlainDictionary::PlainDictionary(HDTSpecification &specification)
    : spec(specification) {
...
std::string PlainDictionary::idToString(unsigned int id,
...
unsigned int PlainDictionary::stringToId(std::string &key,
                                         TripleComponentRole position) {
  DictEntryIt ret;
...
void PlainDictionary::save(std::ostream &output,
  ...
  for (i = 0; i < shared.size(); i++) {
    ...
    counter++;
    NOTIFYCOND(listener, "PlainDictionary saving shared", counter,
               getNumberOfElements());
  }

  output.put(
      marker); // extra line to set the begining of next part of dictionary
  ...
  for (i = 0; i < subjects.size(); i++) {
    ...
    output.put(marker); // character to split file
    counter++;
    NOTIFYCOND(listener, "PlainDictionary saving subjects", counter,
               getNumberOfElements());
  }
  ...
  for (i = 0; i < objects.size(); i++) {
    ...
    output.put(marker); // character to split file
    counter++;
    NOTIFYCOND(listener, "PlainDictionary saving objects", counter,
               getNumberOfElements());
  }
  ...
  for (i = 0; i < predicates.size(); i++) {
    ...
    output.put(marker); // character  to split file
    counter++;
    NOTIFYCOND(listener, "PlainDictionary saving predicates", counter,
               getNumberOfElements());
  }
...
void PlainDictionary::load(std::istream &input, ControlInformation &ci,
  ...
  if (format != getType()) {
    throw "Trying to read a PlainDictionary but the data is not "
          "PlainDictionary";
  }
  ...
  while (region < 5 && getline(input, line, '\1')) {
    ...
    if (line != "") {
      if (region == 1) { // shared SO
        NOTIFYCOND(&iListener, "Dictionary loading shared area.", numLine,
                   numElements);
        insert(line, SHARED_SUBJECT);
      } else if (region == 2) { // not shared Subjects
        NOTIFYCOND(&iListener, "Dictionary loading subjects.", numLine,
                   numElements);
        insert(line, NOT_SHARED_SUBJECT);
        NOTIFYCOND(&iListener, "Dictionary loading objects.", numLine,
                   numElements);
      } else if (region == 3) { // not shared Objects
        insert(line, NOT_SHARED_OBJECT);
        NOTIFYCOND(&iListener, "Dictionary loading predicates.", numLine,
                   numElements);
...
size_t PlainDictionary::load(unsigned char *ptr, unsigned char *ptrMax,
                             ProgressListener *listener) {
...
uint64_t PlainDictionary::size() { return sizeStrings; }

unsigned int PlainDictionary::insert(std::string &str,
                                     TripleComponentRole pos) {
  ...
  if (pos == SUBJECT) {
    if (!foundSubject && !foundObject) {
      ...
      // cout << " Add new subject: " << str << " with entry " << entry->id
      //< endl;
    ...
    if (!foundSubject && !foundObject) {
      ...
      return subjectIt->second->id; // Return
    }
  }
...
void PlainDictionary::split(ProgressListener *listener) {
  ...
  for (DictEntryIt subj_it = hashSubject.begin();
       subj_it != hashSubject.end() && subj_it->first; subj_it++) {
  ...
  for (DictEntryIt obj_it = hashObject.begin(); obj_it != hashObject.end();
       ++obj_it) {
  ...
  {
    {
      sort(predicates.begin(), predicates.end(),
           DictionaryEntry::cmpLexicographic);
    }
...
PlainDictionary::getDictionaryEntryVector(unsigned int id,
                                          TripleComponentRole position) {
  switch (position) {
  case SUBJECT:
    if (id <= shared.size()) {
...
unsigned int PlainDictionary::getGlobalId(unsigned int mapping, unsigned int id,
...
unsigned int PlainDictionary::getGlobalId(unsigned int id,
                                          DictionarySection position) {
  return getGlobalId(this->mapping, id, position);
}

unsigned int PlainDictionary::getLocalId(unsigned int mapping, unsigned int id,
                                         TripleComponentRole position) {
  switch (position) {
  case SUBJECT:
    if (id <= shared.size()) {
...
unsigned int PlainDictionary::getLocalId(unsigned int id,
                                         TripleComponentRole position) {
...
unsigned int PlainDictionary::getMaxPredicateID() { return predicates.size(); }
...
unsigned int PlainDictionary::getNpredicates() { return predicates.size(); }

unsigned int PlainDictionary::getNobjects() {
  return shared.size() + objects.size();
}

unsigned int PlainDictionary::getNshared() { return shared.size(); }

void PlainDictionary::updateID(unsigned int oldid, unsigned int newid,
                               DictionarySection position) {
  switch (position) {
  case SHARED_SUBJECT:
  case SHARED_OBJECT:
...
void PlainDictionary::populateHeader(Header &header, string rootNode) {
  header.insert(rootNode, HDTVocabulary::DICTIONARY_TYPE,
                HDTVocabulary::DICTIONARY_TYPE_PLAIN);
...
unsigned int PlainDictionary::getMapping() { return mapping; }

void PlainDictionary::getSuggestions(const char *base,
